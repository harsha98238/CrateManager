...
void displayMenu(String,String,String,String);
void displayCount();
void Function(char);
...
const unsigned long interval = 1UL*60UL*60UL*1000UL;   // Change  every 60 minute(1 hour)
...
int Object2PresentOutput = 0;
 ...
 /*new crate error*/
unsigned long CratePresenceTimer = 0;
...
int fc_interval=0;
bool fcflag = false;            //full cop flag
bool fc_toggle = false;//full carp detecton 
...
//    digitalWrite(Backlight,HIGH);
  ...
  sprintf(bufferVal,"%-20s","");
  EmptyBuffer = bufferVal;
  displayMenu(EmptyBuffer,EmptyBuffer,EmptyBuffer,EmptyBuffer);
...
unsigned long EntTime = 0;
unsigned long MenuTime = 0;

void loop(){ 

if ((InsideClearFlag == true or InsideMenuFlag == true or LcdRefreshFlag ==true) and millis()-MenuTime >1000)
{
    MenuTime = millis();
...
if (((RefreshTimer - PreviousRefreshTimer)  >=  RefreshInterval) and (LcdRefreshFlag == false) and (InsideMenuFlag == false)) 
 {
...
if (Initializationflag = true and InitCopStorageFlag == false )
{
  ...
  //Serial1.println("cop storage  high at setup");
}
  if( millis() - InitCopStorageTimer2 > InitCopStorageOffDelay*50 and InitCopStorageFlag2 ==  true)
{
  ...
  //Serial1.println("cop storage  low at setup");
}
  ...
  if  (millis() - InitCrateEjectionTimer1 > 2000 and InitCrateEjectionFlag1 == true)
{
  ...
  //Serial1.println("crate eject high at setup");
...
if ( (millis() - beltonDetection > (timeblock)) or (millis() - beltoffDetection > (timeblock)))
  { 
    ...
    //Serial1.println("Vertical belt error occured");
    errorB = true;
  }
...
if ((millis() - fberrtimer > (fberrtimerondelay * 50) and fberrflag1 == true) )
  { 
    FullCopError  = true;
    //Serial1.println("Full bobbin error occured");
    errorFC = true;
    errorB = true;
  }
///*FullCopDetection New*/
// if (digitalRead(fullCopDetection) == false and fc_toggle == true)//low trigger on
//  {
//    fullcopBeltSeperationTimerStart= millis();
//    fc_toggle = false;
//    //if(millis()-fullcopBeltSeperationTimerStart <5000)
//     // beltError = false;
//  }
//  
//  if (digitalRead(fullCopDetection) == true and fc_toggle == false )
//  { 
//    fullcopBeltSeperationTimerEnd = millis();
//    fullcopinterval = fullcopBeltSeperationTimerEnd - fullcopBeltSeperationTimerStart;
//    ////Serial1.println(fullcopinterval);
//    if (fullcopinterval > 300)//250
//      {
//        
//        fc_time = millis();
//      }
...
//  }
//
//  if(fullcopinterval<800 and fullcopinterval>300)//find FC 
//  {
//    fcflag = true;
//    fc_interval = fullcopinterval;
//    fullcopinterval = 0;
//  }
////To Trigger Nozzle after a delay of 300ms
//  if ((millis() - fc_time > 300) and (millis() -fc_time < 500 ) and fcflag == true)//nozzle timing
//  {
//    fcflag = false;//check..
//    //Serial1.println("S1 high");
//    digitalWrite(s1,HIGH);
//    FullCopCountFlag = true; 
//  }  
//  else
//  {
//   if(FullCopCountFlag == true and millis()-fc_time >600)//time??
//    {
//    FullCopCount++;
...
//    digitalWrite(s1,LOW);
//    fc_time = 0;
//    //Serial1.println(FullCopCount);
//    //Serial1.println(fc_interval);
//    }
//  }
///*Fc new end*/
...
//Full Cop detection
  ...
  {
    fullcopBeltSeperationTimerStart= millis();
    ////Serial1.println("full cop sens on");
    togglefc = true;
  }
  ...
  { 
    fullcopBeltSeperationTimerEnd = millis();
    ////Serial1.println("full cop sens off");
    fullcopinterval = fullcopBeltSeperationTimerEnd - fullcopBeltSeperationTimerStart;
    if (fullcopinterval > 200)//300before200
      {
        //Serial1.println("start of Full cop"+String(fullcopBeltSeperationTimerStart));
        //Serial1.println("End of Full cop"+ String(fullcopBeltSeperationTimerEnd));
        //Serial1.println("belt cut time exceeded threshold: " + String(fullcopBeltSeperationTimerEnd - fullcopBeltSeperationTimerStart));
        //Serial1.println("setting timefc");
        timefc = millis();
      }
    togglefc = false;
...
// ****************************Yarn1 identification********************************//
    ...
    timeryarn1 = millis();
    yarn1flag = true;
    //Serial1.println("yarn1 identified");
...
/****************************Color 1 identification********************************/
 if(digitalRead(color1) == false and  color1flag == false) //clr1false
 {
  ...
  //Serial1.println("colour1 identified");
  s2on = true;
...
//    //Serial1.println("Object1 identified");
//    color1flag = false;
...
//TO TRIGGER SHUTTER2 for color1COP
  if (Object1flag1 == true and digitalRead(obj1) == false and color1identified == true and yarn1flag == false)
      ...
      Object1flag1 = false;
      //Serial1.println("trigger s2 for c1 cop");
      }
  ...
  //Passing Shutter2 - cop must be 2nd or 3rd color    
...

     {
      ...
      Object1Detected = false;
      Object1flag1 = false;
      //Serial1.println("Color2 empty Check");
    }
...
if(Object1flag1 == true and digitalRead(obj1) == false and yarn1flag == true)
    {
      ...
      krichi1 =  true;
      //Serial1.println("Color1 krichi");
      
      if (krichi1==true)
      {
      Krichi1Cnt++;
//      Serial.print("krichi 1=");
      //Serial1.println(Krichi1Cnt);
      krichiCntFlag=true;
      krichi1==false;
      }   
      ...
      Object1Detected = false;
      Object1flag1 = false;
      //Serial1.println("Color2 yarn Check");
    }
...
if( (millis()-Object1DetectTmr) > (s2OnDelay*1) and (millis()-Object1DetectTmr) < (s2OnDelay+s2OffDelay)*50 and Shutter2OnReady == true and Tray1EmptyFlag == false and Tray2EmptyFlag == false /*and  Tray3EmptyFlag == false*/  )//was*50
  ...
  // //Serial1.println("C1CountFlag1 = true");
  C1CountFlag1 = true;
  }
else{
  digitalWrite(s2,LOW);
...
if (C1CountFlag1 == true and C1CountFlag2 == false)
{
  ...
  color1Cnt++;
  color1Cntflag++;
  //Serial1.println("inside color 1 loop");
  //Serial.print("color 1 count=");
  //Serial1.println(color1Cnt);
//if(CountDisplayFlag == true)
//{  
//  lcd.setCursor(4,0);
//  lcd.print(color1Cnt);
...
if((color1Cnt>0) and ((color1Cnt %  c1Cntreset) ==  0 ) and flag1CopStorage1 == false and copstorage1loopflag == false) //40
{
 ...
 copstorage1loopflag = true;
  ...
  //Serial1.println("cop storage ready to high ");
...
if (millis() - timer1CopStorage1 > copStorage1Ondelay*50  and copstorage1highflag == true)//50
{
  ...
  //Serial1.println("cop storage  high ");
...
if (copstorage1Shutterflag1 == true and copstorage1Shutterflag2 == false)
{
  ...
  //Serial1.println(copStorage1Count);
}
...
if (digitalRead(crate1) == true and crate1alertcheck == true)
{
  ...
  digitalWrite(binLock1,HIGH);
  //Serial1.println("binlock high");
  
}

if (crate1error == true and digitalRead(crate1) == false) 
{
  CratePresenceTimer = millis();
  Crate1Error2 = true;
  crate1error = false;
}
if ((((millis())-(CratePresenceTimer)) > 5000) and Crate1Error2 == true) 
{
  digitalWrite(binUp1,HIGH);
  Crate1Error2 = false;
...
if( copStorage1Count >= numCopStorage1 and crate1error == false /*and digitalRead(crate1) == false*/)
{
  ...
  Crate1EjectCount++;
  digitalWrite(crateEjector1,HIGH);
  //Serial1.println("crate eject high");
...
if(digitalRead(color2) == false and  Color2flag == false and ColorSensorOption == false and Object2Detected == true ) 
 {
  ...
  //Serial1.println("colour2 identified direct");
  s3on = true;
}
...
//TO TRIGGER SHUTTER3 for color2COP
  if (Object2Detected == true and digitalRead(obj2) == false and color2identified == true and yarn2flag == false and BobbinMixupflag == false)
      {
...
//Color 2 COP with Yarn (color 2 kirchi)
    if   (Object2Detected == true and digitalRead(obj2) == false and color2identified == true and yarn2flag == true)//Object2flag1 == true 
    {
      Shutter3Onflag1 = false;
      Object2DetectTmr = millis();
      yarn2flag = false;
...
if( (millis()-Object2DetectTmr) > (s3OnDelay*50)/* *1 */ and (millis()-Object2DetectTmr) < (s3OnDelay+s3OffDelay)*50 and Shutter3OnReady == true and Tray1EmptyFlag == false and Tray2EmptyFlag == false /*and  Tray3EmptyFlag == false*/ )//was*50
  ...
  ////Serial1.println("C2CountFlag1 = true");
  C2CountFlag1 = true;
  }
else{
  digitalWrite(s3,LOW);
...
if (C2CountFlag1 == true and C2CountFlag2 == false)
{
  C2CountFlag2 = true;
  color2Cnt++;
  //Serial1.println("inside color 2 loop");
  color2Cntflag++;
  //Serial.print("color 2 count=");
  //Serial1.println(color2Cnt);
//  if(CountDisplayFlag == true)
//{
//  lcd.setCursor(14,0);
//  lcd.print(color2Cnt);
...
//  flag2CopStorage2 = true;
  ...
  //Serial1.println("cop storage 2 ready to high ");
...
if (copstorage2Shutterflag1 == true and copstorage2Shutterflag2 == false)
{
  ...
  //Serial1.println(copStorage2Count);
}
...
if (digitalRead(crate2) == true and crate2alertcheck == true)
{
  ...
  crate2error = true;
  digitalWrite(binLock2,HIGH);
  //Serial1.println("binlock 2 high");
}


if (crate2error == true and digitalRead(crate2) == false)  
{
  Crate2PresenceTimer = millis();
  Crate2Error2 = true;
  crate2error = false;
}
if ((((millis())-(Crate2PresenceTimer)) > 5000) and Crate2Error2 == true) 
...
if( copStorage2Count >= numCopStorage2 and crate2error == false /*and digitalRead(crate2) == false */)
{
  ...
  {
  digitalWrite(crateEjector2,HIGH);
  //Serial1.println("crate eject 2 high");
...
//    //Serial1.println("Object3 error occured");
//    errors4 = true;
//    
//  }
//
...
//    //Serial1.println("Yarn3 sensor error occured");
//    error = true;
//   
//  }      
///*s4error ends*/
...
//      Shutter4Onflag1 = false;
////       krichi3 =  true;
////      //Serial1.println("color3 yarn Rested");
//     
//      Object3DetectTmr = millis();
//      yarn3flag = false;
//      yarn3detected = false;
...
//  //Serial1.println("cop storage 3 ready to high ");
//  
//}
//
...
//  //Serial1.println(CopStorage3Count);
//}
...
//  crate3error = true;
//  digitalWrite(binLock3,HIGH);
//  //Serial1.println("binlock 3 high");
//}
//
...
//  {
//  digitalWrite(crateEjector3,HIGH);
//  //Serial1.println("crate eject 3 high");
...
//    lcd.setCursor(0,3);
//    lcd.print("                    ");
    //Serial1.println("Error reset at loop");
    //this is both horizontal and vertical
    digitalWrite(vcMotor,HIGH);
...
{
//Serial1.println("function executed"+customKey);
Function(functionParameter);
Serial.println(functionParameter);
...
Serial.println(customKey);
    //Serial1.println("keypressdflag true"); 
    KeyPressFlag = true;
    MenuTimer = millis();
...
bool printError()
{
 ...
 if(InsideMenuFlag == false and InsideClearFlag == false and LcdRefreshFlag == false)
 {
  switch(temp)
 {
  case 0:
...
//    lcd.print("BELT ERROR");
 sprintf(bufferVal,"%-20s","BELT ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
  }
  else
...
//    lcd.print("FULLCOP ERROR");
  sprintf(bufferVal,"%-20s","FULLCOP ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
...
//    lcd.print("YARN1  ERROR");
  sprintf(bufferVal,"%-20s","YARN1 ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
...
//    lcd.print("COLOR1 ERROR");
  sprintf(bufferVal,"%-20s","COLOR1 ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
...
//    lcd.print("OBJECT1 ERROR");
  sprintf(bufferVal,"%-20s","OBJECT1 ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
...
//    lcd.print("YARN2  ERROR");
  sprintf(bufferVal,"%-20s","YARN2 ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
...
//    lcd.print("COLOR2 ERROR");
  sprintf(bufferVal,"%-20s","COLOR2 ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
...
//    lcd.print("OBJECT2 ERROR");
  sprintf(bufferVal,"%-20s","OBJECT2 ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
...
//    lcd.print("TRAY1 ERROR");
  sprintf(bufferVal,"%-20s","TRAY1 ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
      return true;
...
//    lcd.print("TRAY2 ERROR");
  sprintf(bufferVal,"%-20s","TRAY2 ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
...
//    lcd.print("YARN3  ERROR");
  sprintf(bufferVal,"%-20s","YARN3 ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
...
//    lcd.print("COLOR3  ERROR");
  sprintf(bufferVal,"%-20s","COLOR3 ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
...
//    lcd.print("OBJECT3 ERROR");
  sprintf(bufferVal,"%-20s","OBJECT3 ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
...
//    lcd.print("TRAY3 ERROR");
  ...
  sprintf(bufferVal,"%-20s","TRAY3 ERROR");
  LineBuffer4 = bufferVal;
  displayMenu("","","",LineBuffer4);
    digitalWrite(alarmLightyellow,HIGH);
    return true;
...
void Function(char customKey)
{
 ...
 //Serial1.println(customKey);
 
if (customKey=='D')
{
    ResetSwitch = true;
    ErrorCycle = false;
    //Serial1.println("Error Reset");
  }
...
displayCount();
//  
  //Serial1.println("Done resetting display");
...
//  //Serial1.println("BacklightTimer flag");
...
//  lcd.print("PRESS B TO EXIT");
  ...
  sprintf(bufferVal,"%-20s","CLEAR COUNT?");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS 1 TO CLEAR");
  LineBuffer2 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS B TO EXIT");
  LineBuffer3 = bufferVal;
  LineBuffer4 = "";
  displayMenu(LineBuffer1,LineBuffer2,LineBuffer3,LineBuffer4);
...
//  EEPROM.update(23,0);
  ...
  sprintf(bufferVal,"%-20s","COUNT CLEARED");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS B TO EXIT");
  LineBuffer2 = bufferVal;
...
{
  sprintf(bufferVal,"%-20s","1. BIN SIZE");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","2. BOBBIN MIX");
  LineBuffer2 = bufferVal;
  sprintf(bufferVal,"%-20s","3. SENSOR OPTION");
  LineBuffer3 = bufferVal;
  LineBuffer4 = EmptyBuffer;
  displayMenu(LineBuffer1,LineBuffer2,LineBuffer3,LineBuffer4);
...
{
  sprintf(bufferVal,"%-20s","SELECT BIN SIZE");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","1.140 2.160 3.180");
  LineBuffer2 = bufferVal;
  sprintf(bufferVal,"%-20s","4.200 5.120 6.10");
  LineBuffer3 = bufferVal;
  LineBuffer4 = EmptyBuffer;
  displayMenu(LineBuffer1,LineBuffer2,LineBuffer3,LineBuffer4);
...
{
  sprintf(bufferVal,"%-20s","1. BOBBIN MIX ON");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","2. BOBBIN MIX OFF");
  LineBuffer2 = bufferVal;
  LineBuffer3 = EmptyBuffer;
  LineBuffer4 = EmptyBuffer;
  displayMenu(LineBuffer1,LineBuffer2,LineBuffer3,LineBuffer4);
...
{
  sprintf(bufferVal,"%-20s","1. MULTI COLOUR");
  LineBuffer1 = bufferVal;
...
//  lcd.setCursor(0,2);
//  lcd.print("PRESS B TO EXIT");
  sprintf(bufferVal,"%-20s","Bin Size = 140");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS B TO EXIT");
  LineBuffer2 = bufferVal;
  LineBuffer3 = EmptyBuffer;
...
//  lcd.setCursor(0,2);
//  lcd.print("PRESS B TO EXIT");
  sprintf(bufferVal,"%-20s","Bin Size = 160");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS B TO EXIT");
  LineBuffer2 = bufferVal;
  LineBuffer3 = EmptyBuffer;
...
//  lcd.setCursor(0,2);
//  lcd.print("PRESS B TO EXIT");
  sprintf(bufferVal,"%-20s","Bin Size = 180");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS B TO EXIT");
  LineBuffer2 = bufferVal;
  LineBuffer3 = EmptyBuffer;
...
//  lcd.setCursor(0,2);
//  lcd.print("PRESS B TO EXIT");
  sprintf(bufferVal,"%-20s","Bin Size = 200");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS B TO EXIT");
  LineBuffer2 = bufferVal;
  LineBuffer3 = EmptyBuffer;
...
//  lcd.setCursor(0,2);
//  lcd.print("PRESS B TO EXIT");
  sprintf(bufferVal,"%-20s","Bin Size = 120");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS B TO EXIT");
  LineBuffer2 = bufferVal;
  LineBuffer3 = EmptyBuffer;
...
//  lcd.setCursor(0,2);
//  lcd.print("PRESS B TO EXIT");
  sprintf(bufferVal,"%-20s","Bin Size = 10");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS B TO EXIT");
  LineBuffer2 = bufferVal;
  LineBuffer3 = EmptyBuffer;
...
//  lcd.setCursor(0,2);
//  lcd.print("PRESS B TO EXIT");
  sprintf(bufferVal,"%-20s","BOBBIN MIX ON");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS B TO EXIT");
  LineBuffer2 = bufferVal;
  LineBuffer3 = EmptyBuffer;
...
//  lcd.setCursor(0,2);
//  lcd.print("PRESS B TO EXIT");
  sprintf(bufferVal,"%-20s","BOBBIN MIX OFF");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS B TO EXIT");
  LineBuffer2 = bufferVal;
  LineBuffer3 = EmptyBuffer;
...
//  lcd.setCursor(0,2);
//  lcd.print("PRESS B TO EXIT");
  sprintf(bufferVal,"%-20s","MULTICOLOUR SENSOR");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS B TO EXIT");
  LineBuffer2 = bufferVal;
  LineBuffer3 = EmptyBuffer;
...
//  lcd.setCursor(0,2);
//  lcd.print("PRESS B TO EXIT");
  sprintf(bufferVal,"%-20s","SINGLE COLOUR SENSOR");
  LineBuffer1 = bufferVal;
  sprintf(bufferVal,"%-20s","PRESS B TO EXIT");
  LineBuffer2 = bufferVal;
  LineBuffer3 = EmptyBuffer;
...
{
  //Serial1.println("inside motor off loop");
  digitalWrite(vcMotor,LOW);
  MotorFlag = true;
}
...
//  digitalWrite(vcMotor,HIGH);
//  MotorFlag = false;
//  //Serial1.println("inside motor on loop");
//  
//}
